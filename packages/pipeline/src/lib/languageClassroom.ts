...
import type { LanguageClassroomKeyword, LanguageClassroomLesson } from '../types.js';
...
export function normalizeLanguageClassroomKeywords(
  ...
  return options.maxKeywords === undefined ? keywords : keywords.slice(0, options.maxKeywords);
}

export function normalizeLanguageClassroomKeyword(raw: unknown): LanguageClassroomKeyword | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
