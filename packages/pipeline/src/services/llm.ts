...
export function buildLanguageClassroomUserMessage(input: LanguageClassroomInput): string {
  return [
    ...
    withThinkingModel(
      {
        model,
        messages: [
          { role: 'system', content: languageClassroomSystemPrompt(input.sourceLanguageCode) },
...
function parseLanguageClassroomLessons(
  ...
  const byTargetLanguage = new Map(lessons.map((lesson) => [lesson.targetLanguageCode, lesson]));
  const ordered = targetLanguageCodes
    .map((targetLanguageCode) => byTargetLanguage.get(targetLanguageCode))
    .filter((lesson): lesson is LanguageClassroomLesson => lesson !== undefined);

  if (ordered.length === 0) {
    throw new Error('Language classroom response did not contain any valid lessons');
  }
