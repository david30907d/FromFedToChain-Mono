...
describe('performIngest failure paths', () => {
  ...
  it('leaves the localization at script_generated when TTS fails', async () => {
    ...
    await expect(performIngest('https://example.com/article', 'zh-Hant')).rejects.toThrow(
      '[step:textToSpeech] TTS unavailable',
    );
  ...
  it('does not mark completed when HLS upload fails', async () => {
    ...
    await expect(performIngest('https://example.com/article', 'zh-Hant')).rejects.toThrow(
      '[step:uploadHlsToR2] R2 upload failed',
    );
  ...
  it('returns the episode when language classroom generation fails', async () => {
    ...
    mockGenerateLanguageClassroomsWithLLM.mockRejectedValue(new Error('LLM timeout'));

    const result = await performIngest('https://example.com/article', 'zh-Hant');
  ...
  it('refreshes article content when an existing localization is pending', async () => {
    ...
    expect(mockUpdateEpisodeLocalizationArticleContent).toHaveBeenCalledWith(localizationRow().id, {
      title: '軟體更新',
      text: '滑鼠和腳踏車市場',
    });
    expect(mockUpdateEpisodeLocalizationStatus).toHaveBeenCalledWith(
  ...
  it('returns ordered existing classrooms without calling the LLM', async () => {
    ...
    expect(result.episode.languageClassrooms.map((lesson) => lesson.targetLanguageCode)).toEqual([
      'ja',
      'en',
      'ko',
    ]);
  ...
  it('wraps non-Error step failures', async () => {
    ...
    await expect(performIngest('https://example.com/article', 'zh-Hant')).rejects.toThrow(
      '[step:scrapeArticle] network down',
    );
  });
  ...
  it('preserves provider metadata on wrapped errors', async () => {
    ...
    await expect(performIngest('https://example.com/article', 'zh-Hant')).rejects.toMatchObject({
  ...
  it('throws when a localization cannot be created', async () => {
    ...
    await expect(performIngest('https://example.com/article', 'zh-Hant')).rejects.toThrow(
      'Failed to create episode localization',
    );
  });
  ...
  it('throws when script generation status update returns no localization', async () => {
    ...
    await expect(performIngest('https://example.com/article', 'zh-Hant')).rejects.toThrow(
      'Failed to retrieve episode localization',
    );
  });
});
