...
const expectedUserStateReadColumns = ['user_id', 'episode_id', 'listened', 'last_position_seconds'];

describe('Supabase user_episode_state grants', () => {
  it('keeps schema.sql aligned with mobile feed state reads', () => {
    const schema = readRepoFile('packages/pipeline/supabase/schema.sql');
...
function mobileUserEpisodeStateSelectColumns(): string[] {
  ...
  const pattern = /\.from\('user_episode_state'\)\s*\.select\(\s*'([^']+)'\s*\)/g;
...
function readSortedMigrations(): string[] {
  const migrationsDir = path.join(repoRoot, 'packages/pipeline/supabase/migrations');
  return fs
